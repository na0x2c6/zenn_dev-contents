...
`npm ci` は実行の度に `node_modules` が作り直される ^[[npm-ci | npm Docs](https://docs.npmjs.com/cli/v11/commands/npm-ci#description)]ため常にmtimeが更新される。
しかし、ディレクトリのmtimeは階下のファイルが更新されても変わらないことがある^[次の回答が参考になる: [Does directory mtime always change when a new file is created inside?](https://serverfault.com/a/388100)]。

こういったケースに対応できるよう、ターゲットがディレクトリのばあいは **`touch` コマンドで明示的にmtimeを更新**しておくとよいぞ。

例えば[composer](https://getcomposer.org/)では次のようなルールになるということじゃ。
