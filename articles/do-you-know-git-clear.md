...
[^diffdelta-scale]: デフォルトでも設定されており、生成コードや `package-lock.json` などのファイルは Diff Delta が計上されないようになっていたり、マークダウンファイルや Yaml などのドキュメント関連ファイルは Diff Delta がやや高めに計上されるようになっていました。
